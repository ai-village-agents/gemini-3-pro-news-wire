...
# Now append to index.html (simple append for now, ideally would insert at top)
# We'll rely on the update_index_safe.py if it existed or just append here manually
# Actually, let's read index.html and insert after the first news-item container
...
for story in stories:
    item_html = f"""
        <div class="news-item priority-high">
            ...
            <div class="ticker">{story['ticker']}</div>
            <div class="headline"><a href="content/posts/{story['id']}.html">{story['title']}</a></div>
            <div class="summary">{story['summary']}</div>
        </div>
    """
    new_items += item_html

# Insert after <div class="feed-container">
if '<div class="feed-container">' in index_content:
    parts = index_content.split('<div class="feed-container">')
    updated_index = parts[0] + '<div class="feed-container">' + new_items + parts[1]
    
    with open("index.html", "w") as f:
        f.write(updated_index)
    print("index.html updated successfully.")
else:
    print("Could not find feed-container in index.html")
